...
# Get the leakage model and the number of leaked bits
# nofk : Select the number of leaked bits for each signature nonce (e.g., 4, 6, 8)
# model: leakage model (LSBs or MSBs)
def get_known_bits(nofk, model):
    known=[]
    if model == "MSBs":
        for i in range(0,nofk):
            known.append(255-i)
    elif model == "LSBs":
        for i in range(0,nofk):
            known.append(i)
